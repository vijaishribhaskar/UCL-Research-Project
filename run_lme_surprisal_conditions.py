# -*- coding: utf-8 -*-
"""
Linear Mixed Effects (LME) Models: Surprisal × Conditions → Reading Times
===========================================================================
Independent Variables (IVs):
  - Surprisal   : continuous, z-scored
  - VerbType    : CP / DP / unacc  (treatment coding, ref = CP)
  - Structure   : move / nomove    (treatment coding, ref = move)
...
Random effect: by-Participant random intercept, by-item random intercept
"""
...
# Keep needed columns
cols = ['Participant', 'item', 'VerbType', 'structure',
...
# Set categorical reference levels: CP and move
...
print(f"VerbType levels  : {df_verb['VerbType'].cat.categories.tolist()}")
print(f"Structure levels : {df_verb['structure'].cat.categories.tolist()}")

# 2. First-Pass dataset
df_fp = df_verb[['Participant', 'item', 'VerbType', 'structure',
                  'Surprisal', 'verb_log_freq', 'IA_FIRST_RUN_DWELL_TIME']].dropna()
...
df_fp['group']       = 1
print(f"\nFirst-Pass dataset      : {len(df_fp)} observations")

# 3. Regression-Path dataset
df_rp = df_verb[['Participant', 'item', 'VerbType', 'structure',
                  'Surprisal', 'verb_log_freq', 'IA_REGRESSION_PATH_DURATION']].dropna()
...
df_rp['group']       = 1
print(f"Regression-Path dataset : {len(df_rp)} observations")
...
# 4. Model formula
formula_fp = (
    "log_fp ~ freq_z + Surprisal_z "
    "* C(VerbType, Treatment('CP')) "
    "* C(structure, Treatment('move'))"
)
formula_rp = (
    "log_rp ~ freq_z + Surprisal_z "
    "* C(VerbType, Treatment('CP')) "
    "* C(structure, Treatment('move'))"
)

# 5. Model 1 – First-Pass Reading Time
print("\n" + "=" * 72)
print("Fitting Model 1: Surprisal x Conditions -> First-Pass RT...")
vc_formula = {
...
hdr_fp = (
    ...
    "IVs: freq_z + Surprisal_z x VerbType (ref=CP) x Structure (ref=move)\n"
...
# 6. Model 2 – Regression-Path Reading Time
print("\n" + "=" * 72)
print("Fitting Model 2: Surprisal x Conditions -> Regression-Path RT...")
...
# 7. Descriptive cell means
print("\n" + "=" * 72)
print("DESCRIPTIVE MEANS BY CONDITION (raw ms)")
print("=" * 72)

print("\nFirst-Pass RT (ms) — VerbType × Structure:")
print(df_fp.groupby(['VerbType', 'structure'])['IA_FIRST_RUN_DWELL_TIME']
        .agg(['count', 'mean']).round(1).unstack().to_string())

print("\nRegression-Path RT (ms) — VerbType × Structure:")
print(df_rp.groupby(['VerbType', 'structure'])['IA_REGRESSION_PATH_DURATION']
        .agg(['count', 'mean']).round(1).unstack().to_string())

# 8. Save results
output_path = 'lme_surprisal_conditions_results.txt'
with open(output_path, 'w', encoding='utf-8') as f:
    f.write("LME MODELS: Surprisal x Conditions -> Reading Times\n")
    f.write("=" * 72 + "\n\n")
    f.write("IVs: freq_z + Surprisal (z-scored) x VerbType (CP/DP/unacc, ref=CP)\n"
            "           x Structure (move/nomove, ref=move)\n")
    ...
    f.write(df_fp.groupby(['VerbType', 'structure'])['IA_FIRST_RUN_DWELL_TIME']
              .agg(['count', 'mean']).round(1).unstack().to_string())
    f.write("\n\nRegression-Path RT (ms):\n")
    f.write(df_rp.groupby(['VerbType', 'structure'])['IA_REGRESSION_PATH_DURATION']
              .agg(['count', 'mean']).round(1).unstack().to_string())
